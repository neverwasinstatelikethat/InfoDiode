...
def _load_xlsx(filepath: Path) -> list[dict]:
    ...
    for sheet_name in sheet_names:
        ...
        for row in rows[header_row_idx + 1:]:
            ...
            # КРИТИЧНО: Пропускаем технические английские заголовки (DESCRIPTION_ANALOG, DEM, etc.)
            if _is_technical_header_row(row):
                logger.debug("Лист '%s': пропущен технический заголовок: %s", sheet_name, row[:5])
                continue
...
def _detect_header_row(rows: list, max_rows: int = 5) -> int:
    ...
    КРИТИЧНО: Пропускает технические английские заголовки (DESCRIPTION_ANALOG, DEM, etc.)
    Ищет строки с русскими названиями параметров или ключевыми словами заголовков.
    ...
    """
    # Ключевые слова технических заголовков (ПРОПУСКАТЬ)
    TECH_HEADERS = {
        'description_analog', 'description_discrete', 'description_command',
        'description_status', 'dem', 'min', 'max', 'name', 'id',
        'unit', 'type', 'sensor', 'decimal', 'short',
    }
    
    # Ключевые слова реальных заголовков (ИСКАТЬ)
    REAL_HEADER_KEYWORDS = {
        'наименование', 'описание', 'название', 'параметр', 'имя',
        'единиц', 'измерен', 'размерность', 'ед.',
        'коротк', 'сокращ', 'код', 'тег', 'обознач', 'тип',
        'знак', 'точн', 'разряд', 'номер', '№',
    }
    ...
    for row_idx in range(min(max_rows, len(rows))):
        ...
        for cell in row:
            ...
            text_cells.append(cell_lower)
            
            # Проверяем на технический заголовок
            if cell_lower in TECH_HEADERS or any(kw in cell_lower for kw in ['description_', 'analog', 'discrete']):
                has_tech_header = True
            
            # Проверяем на реальный заголовок
            if any(kw in cell_lower for kw in REAL_HEADER_KEYWORDS):
                has_real_header = True
        
        # Логика определения:
        # 1. Если есть русские ключевые слова — ЭТО заголовок
        if has_real_header and len(text_cells) >= 2:
            return row_idx
        
        # 2. Если есть технические слова (DESCRIPTION_ANALOG) — ПРОПУСКАЕМ
        if has_tech_header:
            continue
        
        # 3. Если просто 2+ текстовых ячейки (без тех. слов) — возможно заголовок
        if len(text_cells) >= 2:
            return row_idx
...
def _is_technical_header_row(row: tuple) -> bool:
    """Проверяет, является ли строка техническим заголовком (DESCRIPTION_ANALOG, DEM, etc.).

    Технические заголовки — это английские метаданные из Excel, которые НЕ должны
    попадать в таблицу параметров как реальные параметры.

    Args:
        row: Кортеж значений из Excel строки.

    Returns:
        True если это технический заголовок (пропустить), False если это данные.
    """
    # Ключевые слова технических заголовков
    TECH_HEADERS = {
        'description_analog', 'description_discrete', 'description_command',
        'description_status', 'dem', 'min', 'max', 'name', 'id',
        'unit', 'type', 'sensor', 'decimal', 'short',
    ...
    for cell in row:
        ...
        # Проверяем на технический заголовок
        if cell_lower in TECH_HEADERS:
            return True
        
        # Проверяем на паттерны типа 'description_*'
        if any(kw in cell_lower for kw in ['description_', 'analog', 'discrete', 'command', 'status']):
            return True
...
def _detect_header_row_csv(rows: list, max_rows: int = 5) -> int:
    ...
    КРИТИЧНО: Пропускает технические английские заголовки.
    Использует ту же логику что и _detect_header_row для Excel.
